package site.halenspace.pocketcloud.redis.templete;
...
public class RedisRepository {
    ...
    public <T> T cast(Object obj, Class<T> clazz) {
        ...
        if (clazz == null || !clazz.isInstance(obj)) {
            throw new ClassCastException("the arguments type mismatch cannot be cast");
        }
        return (T) obj;
    }
    ...
    /*public void setExpire(final String key, final Object value, final long time) {
        redisTemplate.execute((RedisCallback<Void>) redisConn -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] keys = serializer.serialize(key);
            byte[] values = OBJECT_SERIALIZER.serialize(value);
            redisConn.setEx(keys, time, values);
            return null;
        });
        log.debug("RedisTemplate set expire cache key: [{}] ", key);
    }*/
    ...
    public <T> T get(final String key, final Class<T> clazz) {
        return cast(redisTemplate.opsForValue().get(key), clazz);
    }

    /*public <T> T get(String key) {
        T result = redisTemplate.execute((RedisCallback<T>) redisConn -> {
            RedisSerializer<String> serializer = this.getRedisSerializer();
            byte[] keys = serializer.serialize(key);
            byte[] values = redisConn.get(keys);
            return (T) OBJECT_SERIALIZER.deserialize(values);
        });
        log.debug("RedisTemplate get cache key: [{}] ", key);
        return result;
    }*/
    ...
    public <T> T getHashValue(String key, String hashKey, Class<T> clazz) {
        return cast(opsForHash().get(key, hashKey), clazz);
    }
     ...
     * @return the hash value
     */
    public <HK, HV> Map<HK, HV> getHashValues(String key) {
        return (Map<HK, HV>) opsForHash().entries(key);
    }
