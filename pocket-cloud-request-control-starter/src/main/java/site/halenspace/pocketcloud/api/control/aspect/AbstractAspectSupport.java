...
import site.halenspace.pocketcloud.api.control.exception.IdempotentException;
...
//    }
    ...
    /*private Method extractDefaultFallbackMethod(ProceedingJoinPoint pjp, String defaultFallback,
                                                Class<?>[] locationClass) {
        if (StrUtil.isBlank(defaultFallback)) {
    ...
    private Method extractBlockHandlerMethod(ProceedingJoinPoint pjp, String name, Class<?>[] locationClass) {
        ...
        MethodWrapper m = ResourceMetadataRegistry.lookupFallback(clazz, name);
        if (m == null) {
            // First time, resolve the block handler.
            Method method = resolveHandlerInternal(pjp, name, clazz, mustStatic);
            // Cache the method instance.
            ResourceMetadataRegistry.updateFallbackFor(clazz, name, method);
            return method;
        }
        if (!m.isPresent()) {
            return null;
